document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function handleLeaveForm() {
           ...
           if (hasOverlappingRequest(startDate, endDate)) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    form.reset();
                    setTimeout(() => window.location.href = '/leaves-view.html', 800);
                } else {
                    const error = await response.json();
                    showError(error.message || 'Failed to submit leave request');
                }
            } catch (e) {
                showError('An error occurred while submitting request');
            } finally {
                isSubmitting = false;
            }
        });
        ...
        cancelButton.addEventListener('click', () => {
            window.location.href = '/leaves-view.html';
        });
    }
        ...
        function renderAll() {
            ...
            leaveRequestsList.querySelectorAll('.btn-edit').forEach(button => {
                button.addEventListener('click', function (e) {
                    e.stopPropagation();
                    const id = this.dataset.id;
                    window.location.href = `/create-leave.html?leaveId=${id}`;
                });
            });
        ...
        if (deleteConfirmButton) {
            deleteConfirmButton.addEventListener('click', async function () {
                ...
                try {
                    ...
                    console.error(e);
                    showError('An error occurred while deleting leave request');
                } finally {
                    closeDeleteModal();
                }
            });
